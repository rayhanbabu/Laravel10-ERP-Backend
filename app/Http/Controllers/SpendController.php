...
class SpendController extends Controller
{
    ...
    public function spend_view(Request $request){
         try{  
            $dept_id = $request->header('dept_id');
            $teacher_id = $request->header('id');
            $scategory=Scategory::where('dept_id',$dept_id)->where('scategory_status',1)->orderby('id','asc')->get();
            $site=Site::where('dept_id',$dept_id)->where('site_status',1)->orderby('id','asc')->get();
            return view('admin.spend',['scategory'=>$scategory,'site'=>$site]);
      ...
      if($validator->fails()){
        ...
        $year = date('Y', strtotime($date));
             ...
             $model->date=$date;
             $model->scategory_id=$request->input('scategory_id');
    ...
    public function spend_edit(Request $request) {
      $id = $request->id;
      $data = Spend::find($id);
      return response()->json([
          'status'=>200,  
    ...
    public function spend_update(Request $request ){
   ...
   public function spend_delete(Request $request) { 
   ...
   public function fetch(Request $request){
       ...
       ->where('spends.dept_id',$dept_id)->select('sites.site_name','scategories.scategory_name'
       ,'teachers.teacher_name','spends.*')
       ->orderBy('id','desc')->paginate(10);
       return view('admin.spend_data',compact('data'));
    }
   ...
   function fetch_data(Request $request)
   {
    if($request->ajax())
    {
          ...
          $data =Spend::leftjoin('sites','sites.id', '=','spends.site_id')
              ->leftjoin('scategories','scategories.id', '=','spends.scategory_id')
              ->leftjoin('teachers','teachers.id', '=','spends.created_by')
              ->where('spends.dept_id',$dept_id)
              ->where(function($query) use ($search) {
                  $query->where('date', 'like', '%'.$search.'%')
                     ->orWhere('amount', 'like', '%'.$search.'%');
               })->select('sites.site_name','scategories.scategory_name'
               ,'teachers.teacher_name','spends.*')->paginate(10);
                   return view('admin.spend_data', compact('data'))->render();
